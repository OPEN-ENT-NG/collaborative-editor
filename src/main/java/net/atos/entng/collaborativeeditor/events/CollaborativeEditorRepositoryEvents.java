...
public class CollaborativeEditorRepositoryEvents implements RepositoryEvents {
	...
	public CollaborativeEditorRepositoryEvents(Vertx vertx, EtherpadHelper helper) {
		this.vertx = vertx;
		this.client = helper.getFirstClient();
	}
	...
	private void exportFiles(final JsonArray results, String exportPath, Set<String> usedFileName,
		...
		if (results.isEmpty()) {
			...
			final String endFileName = fileName;
			...
			vertx.fileSystem().writeFile(filePath, resources.toBuffer(), new Handler<AsyncResult<Void>>() {
				...
				public void handle(AsyncResult<Void> event) {
					if (event.succeeded()) {
						exportPad(fileId, exportPath, endFileName, new Handler<Boolean>() {
							@Override
	...
	private void exportPad(String padId, String exportPath, String fileName, Handler<Boolean> handler) {
		final String padPath = exportPath + File.separator + "Pad_" + fileName;
	...
	public void exportResources(String exportId, String userId, JsonArray g, String exportPath, String locale,
		...
		if (apps.contains("collaborativeeditor")) {
